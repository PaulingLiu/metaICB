...
tibble(CD8A = tcell.filt@assays$RNA@scale.data["CD8A",]) %>%
  ggplot(aes(CD8A)) +
...
tibble(a = tcell.filt@assays$RNA@scale.data["CXCL13",]) %>%
  ggplot(aes(a)) +
...
tex.clones <- pda %>%
  ...
  as.tibble() %>%
  dplyr::mutate(clone = ifelse(CD8A > 0.5 & CXCL13 > 0.6, "Tex", "Other")) %>%
