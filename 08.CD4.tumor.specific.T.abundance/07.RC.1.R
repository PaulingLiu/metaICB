...
cdg %>%
  dplyr::filter(gene_name %in% c("CXCL13","CD8A", "CD4"))

cal.genes <- c("ENSG00000156234","ENSG00000153563","ENSG00000010610")
...
pda <- readr::read_rds("/home/pauling/projects/07_pan.cancer.treatment/02.data/09.CD4.analysis/01.clone.expr/08.RC.1.rds.gz")
...
tex.clones.pro <- pda %>%
  ...
  as.tibble() %>%
  dplyr::mutate(clone = ifelse(CD4 > 0.4 & CXCL13 > 0.8, "Tex", "Other"))

tex.clones.pro %>%
  dplyr::filter(CD4 > 0.4) %>%
...
pro.pda %>%
  ...
  geom_jitter(aes(color = response), size = 2, width = 0.15) +
  theme_classic() +
  theme(
    legend.position = "none",
...
t.test(pro.pda$freq[pro.pda$resp == uniq.resp[1]], pro.pda$freq[pro.pda$resp == uniq.resp[2]])
