...
pda <- readr::read_rds("/home/pauling/projects/07_pan.cancer.treatment/02.data/09.CD4.analysis/01.clone.expr/06.BC.batch2.rds.gz")
...
tex.clones.pro <- pda %>%
  ...
  as.tibble() %>%
  dplyr::mutate(clone = ifelse(CD4 > 0.25 & CXCL13 > 0.2, "Tex", "Other"))
...
t.test(pro.pda$freq[pro.pda$resp == uniq.resp[3]], pro.pda$freq[pro.pda$resp == uniq.resp[1]])
...
tex.clones.pro <- pda %>%
  tibble::rownames_to_column(var = "cloneid") %>%
  as.tibble() %>%
  dplyr::mutate(clone = ifelse(CD8A > 0.5 & CXCL13 > 0.1, "Tex", "Other"))
