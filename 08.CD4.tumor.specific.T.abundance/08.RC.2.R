...
pda <- readr::read_rds("/home/pauling/projects/07_pan.cancer.treatment/02.data/09.CD4.analysis/01.clone.expr/09.RC.2.rds.gz")
...
tex.clones.pro <- pda %>%
  dplyr::mutate(clone = ifelse(type == "CD4" & CXCL13 > 0.5, "Tex", "Other"))

tex.clones.pro %>%
  dplyr::filter(type == "CD4") %>%
