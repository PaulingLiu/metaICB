...
FeaturePlot(sce.sub, features = c("CXCL13","FOXP3","HAVCR2","IL7R","PDCD1","TCF7"), ncol = 3)
FeaturePlot(sce.sub, features = c("TCF7"))

table(Idents(sce.sub), sce.sub$patient)
markers <- FindMarkers(sce.sub, ident.1 = 9, only.pos = T)
sce.sub <- sce.sub[,Idents(sce.sub) != 9]

sce.sub %>% readr::write_rds("/home/pauling/projects/07_pan.cancer.treatment/02.data/10.CD4.tumor.reactive.T.clustering/03.BCC.rds.gz", compress = "gz")
