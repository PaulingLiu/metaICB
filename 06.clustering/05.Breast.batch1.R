...
pda <- readr::read_rds("/home/pauling/projects/07_pan.cancer.treatment/02.data/00.all.tmp.cxcl13.cd8.clone/05.breast.cancer.batch1.rds.gz")
pda <- pda %>% 
...
sce.pro.sub <- bbknn.batch(sce.pro.sub, r = 2, n.pc = 20)

DimPlot(sce.pro.sub, label = T)
FeaturePlot(sce.pro, features = c("HAVCR2","HSPA1A","GZMK","IL7R"))

#---------- Annotation

clusters <- Idents(sce.pro.sub)
cluster.name <- rep("cells", length(clusters))
cluster.name[clusters %in% c(4)] <- "0.GZMK+Texp"
cluster.name[clusters %in% c(3,5,8)] <- "Prolif."
cluster.name[cluster.name == "cells"] <- "TD"

sce.pro.sub$clustesrs <- Idents(sce.pro.sub)
Idents(sce.pro.sub) <- cluster.name

sce.pro.sub %>% readr::write_rds("projects/07_pan.cancer.treatment/01.scRNAseq.collection/04.breast.pdl1/01.tumor.reactive.cells.batch1.rds.gz", compress = "gz")
